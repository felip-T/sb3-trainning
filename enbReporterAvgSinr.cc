    ...
    if (m_active)
    {
            ...
            GetAllRegisteredUeIds(cellId, data);
        {
            for (auto ue : connectedUes)
            {
                ...
                sinrUeReport->SetAttribute("cellid", UintegerValue(cellId));
                sinrUeReport->SetAttribute("RNTI", UintegerValue(rnti));
        ...
        else
        {
            if (!connectedUes.empty())
            {
                m_sinrAvg = m_sinrAvg.Get() / connectedUes.size();
                m_rsrpAvg = m_rsrpAvg.Get() / connectedUes.size();
            }
        }
        ...
        sinrReport->SetAttribute("numUes", UintegerValue(connectedUes.size()));
